...
@Component({
  ...
  template: `
    <div class="bg-white rounded-2xl shadow-xl p-8">
      ...
      <p class="text-sm text-slate-500 mb-6">Register as an operator</p>

      <form [formGroup]="form" (ngSubmit)="submit()" class="space-y-4" novalidate>

        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1.5" for="reg-email">
            Email address
          </label>
        ...
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1.5" for="reg-password">
            Password
          </label>
          ...
          @if (touched('password') && invalid('password')) {
            <p class="mt-1 text-xs text-red-600">Minimum 8 characters</p>
          }
        </div>
...
export class RegisterComponent {
  ...
  submit(): void {
    ...
    this.auth.register(email!, password!).subscribe({
      next: () => { this.toast.success('Account created — please sign in'); this.router.navigate(['/login']); },
